...
class ScheduleJob():
    ...
    def schedule_job(cls):
        def watch_job():
            ...
            print("**************Begin line*****", begin_line)
            print("**************end line*****", line_end)
                ...
                for k,v in sec_data_dict.items():
                    if len(v)>10:
                        with open(filename_, 'a') as file:  # Use file to refer to the file object
                            file.write("\n")
                            file.write("High traffic generated an alert - hits = {0}, triggered at {1}".format(len(v), k))
                            # i+1 because now last line because now extra line will be added
                            os.environ['LAST_LINE_PROCESSED'] = str(line_end+1)
                    else:
